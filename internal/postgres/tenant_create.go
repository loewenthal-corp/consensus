...
func (_c *TenantCreate) SetKey(v string) *TenantCreate {
	_c.mutation.SetKey(v)
	return _c
}

// SetNillableKey sets the "key" field if the given value is not nil.
func (_c *TenantCreate) SetNillableKey(v *string) *TenantCreate {
	if v != nil {
		_c.SetKey(*v)
	}
	return _c
...
func (_c *TenantCreate) check() error {
	if _, ok := _c.mutation.Key(); !ok {
		return &ValidationError{Name: "key", err: errors.New(`postgres: missing required field "Tenant.key"`)}
	}
	if v, ok := _c.mutation.Key(); ok {
		if err := tenant.KeyValidator(v); err != nil {
			return &ValidationError{Name: "key", err: fmt.Errorf(`postgres: validator failed for field "Tenant.key": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`postgres: missing required field "Tenant.name"`)}
	}
	if v, ok := _c.mutation.Name(); ok {
		if err := tenant.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`postgres: validator failed for field "Tenant.name": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`postgres: missing required field "Tenant.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`postgres: missing required field "Tenant.updated_at"`)}
	}
	return nil
}
...
func (u *TenantUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("postgres: missing options for TenantCreate.OnConflict")
	}
...
func (u *TenantUpsertOne) ID(ctx context.Context) (id uuid.UUID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("postgres: TenantUpsertOne.ID is not supported by MySQL driver. Use TenantUpsertOne.Exec instead")
	}
...
func (u *TenantUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("postgres: OnConflict was set for builder %d. Set it on the TenantCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("postgres: missing options for TenantCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
