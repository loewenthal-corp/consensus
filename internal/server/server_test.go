...
func TestServer_HealthAndAdmin(t *testing.T) {
	...
	require.Contains(t, adminHTML, `name="context"`)
	require.Contains(t, adminHTML, `name="include_upstreams"`)
	require.Contains(t, adminHTML, "Recent Insights")

	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodGet, "/admin/?query=posthog&tags=build&context=tool%3Dturbo&limit=5&include_upstreams=true", nil)
	handler.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
	adminHTML = rec.Body.String()
	require.Contains(t, adminHTML, "search backend is not configured")
	require.Contains(t, adminHTML, `value="posthog"`)
	require.Contains(t, adminHTML, `value="build"`)
	require.Contains(t, adminHTML, `value="tool=turbo"`)
...
func TestServer_MCPListsAllowlistedTools(t *testing.T) {
	...
	require.Contains(t, string(respBody), "small MCP component")
	...
	require.Contains(t, text, "one-shot retrieval tool")
